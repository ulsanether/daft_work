...
void setup() {
  ...
  if (EEPROM.read(0) == 0 and EEPROM.read(1) == 0 and EEPROM.read(2) == 0 and EEPROM.read(3) == 0 and EEPROM.read(4) == 0 and EEPROM.read(5) == 0 and EEPROM.read(6) == 0) {
    Serial.println("서보 정상");
  } else {
    reset_servo();
    Serial.println("리셋 서보 정상");
  }
