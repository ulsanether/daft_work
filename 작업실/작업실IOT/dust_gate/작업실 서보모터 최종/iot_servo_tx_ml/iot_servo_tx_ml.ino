...
#define off_delay 1000 //전류값을 읽고 공구가 차단 되기까지 지연 시간, 집진기가 꺼지는 시간
...
void loop() {
  ...
  if (acs_current > current_max) { //최대 전류 값이 한계를 초과할 경우 상태를 low로 설정
    Serial.println("밀링 ON.");
    delay(800);
    Serial.println(acs_current);
    if (acs_state == 1) {
      char text[] = "ML01"; // 송신할 문자
    ...
    if (acs_state == 0) {
      Serial.println("밀링 OFF.");
      delay(off_delay);
      char text1[] = "ML00"; // 송신할 문자
      radio.write(&text1, sizeof(text1)); // 위 문자를 문자 길이 만큼 송출함
      acs_state = 1;
       Serial.println(acs_state);
    }
  }
}
