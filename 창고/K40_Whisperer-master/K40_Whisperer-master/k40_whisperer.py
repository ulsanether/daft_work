...
##        self.unsharp_t.set("3")
        ...
        # Buttons
        self.Reng_Button  = Button(self.master,text="레스터 방식 각인", command=self.Raster_Eng)
        self.Veng_Button  = Button(self.master,text="백터 방식 각인", command=self.Vector_Eng)
        self.Vcut_Button  = Button(self.master,text="백터 방식 컷팅"    , command=self.Vector_Cut)
        self.Grun_Button  = Button(self.master,text="G-Code 실행"    , command=self.Gcode_Cut)


        self.Reng_Veng_Button      = Button(self.master,text="레스터와 \n백터 방식 각인", command=self.Raster_Vector_Eng)
        self.Veng_Vcut_Button      = Button(self.master,text="백터 각인과\n컷팅", command=self.Vector_Eng_Cut)
        self.Reng_Veng_Vcut_Button = Button(self.master,text="레스터와\n백터각인\n그리고\n백터 컷팅", command=self.Raster_Vector_Cut)
        
        self.Label_Position_Control = Label(self.master,text="위치 컨트롤:", anchor=W)
        
        self.Initialize_Button = Button(self.master,text="레이저 커터 초기화", command=self.Initialize_Laser)

        self.Open_Button       = Button(self.master,text="디자인 파일\n열기",   command=self.menu_File_Open_Design)
        self.Reload_Button     = Button(self.master,text="디자인 파일\n새로고침", command=self.menu_Reload_Design)
        
        self.Home_Button       = Button(self.master,text="원점",            command=self.Home)
        self.UnLock_Button     = Button(self.master,text="레일 잠금 풀기",     command=self.Unlock)
        self.Stop_Button       = Button(self.master,text="잠시 멈춤/정지",      command=self.Stop)
        ...
        self.Label_Step   = Label(self.master,text="스탭 이동거리", anchor=CENTER )
        ...
        ###########################################################################
        self.GoTo_Button    = Button(self.master,text="이동", command=self.GoTo)
        ...
        self.Label_Advanced_column = Label(self.master,text="전문가 설정",anchor=CENTER)
        self.separator_adv = Frame(self.master, height=2, bd=1, relief=SUNKEN)       

        self.Label_Halftone_adv = Label(self.master,text="하프톤(Dither)")
        ...
        self.Label_Negate_adv = Label(self.master,text="레스터 색상 반전")
        ...
        self.Label_Mirror_adv = Label(self.master,text="이미지 대칭")
        ...
        self.Label_Rotate_adv = Label(self.master,text="이미지 회전")
        ...
        self.Label_inputCSYS_adv = Label(self.master,text="CSYS를 사용해서 입력")
        ...
        self.Label_Inside_First_adv = Label(self.master,text="안쪽 먼저 컷팅")
        ...
        self.Label_Rotary_Enable_adv = Label(self.master,text="이미지 회전 설정")
        ...
        self.Label_Comb_Engrave_adv = Label(self.master,text="각인 방식 모음")
        ...
        self.Label_Comb_Vector_adv = Label(self.master,text="백터 방식 모음")
        ...
        self.Label_Reng_passes = Label(self.master,text="레스터 각인. Passes")
        ...
        self.Label_Veng_passes = Label(self.master,text="백터 각인. Passes")
        ...
        self.Label_Vcut_passes = Label(self.master,text="백터 컷. Passes")
        ...
        self.Hide_Adv_Button = Button(self.master,text="전문가 설정 닫기", command=self.Hide_Advanced)
        ...
        top_File.add("command", label = "설정 파일 저장", command = self.menu_File_Save)
        top_File.add("command", label = "설정 파일 읽기", command = self.menu_File_Open_Settings_File)

        top_File.add_separator()
        top_File.add("command", label = "디자인 열기 (SVG/DXF/G-Code)"  , command = self.menu_File_Open_Design)
        top_File.add("command", label = "디자인 다시 불러오기"          , command = self.menu_Reload_Design)

        top_File.add_separator()    
        top_File.add("command", label = "레이저로 EGV 파일 전송"             , command = self.menu_File_Open_EGV)

        SaveEGVmenu = Menu(self.master, relief = "raised", bd=2, tearoff=0)
        top_File.add_cascade(label="EGV 파일 저장", menu=SaveEGVmenu)        
        SaveEGVmenu.add("command", label = "레스터 이미지 각인"     , command = self.menu_File_Raster_Engrave)
        SaveEGVmenu.add("command", label = "백터 이미지 각인"     , command = self.menu_File_Vector_Engrave)
        SaveEGVmenu.add("command", label = "백터 이미지 컷팅"         , command = self.menu_File_Vector_Cut)
        SaveEGVmenu.add("command", label = "G-Code 작동"  , command = self.menu_File_G_Code)
        SaveEGVmenu.add_separator()   
        SaveEGVmenu.add("command", label = "레스터, 백터 이미지 각인"             , command = self.menu_File_Raster_Vector_Engrave)
        SaveEGVmenu.add("command", label = "백터 이미지 각인과 컷팅"                , command = self.menu_File_Vector_Engrave_Cut)
        SaveEGVmenu.add("command", label = "레스터, 백터 이미지 각인과 백터 이미지 컷팅" , command = self.menu_File_Raster_Vector_Cut)
        
    
        top_File.add_separator()
        top_File.add("command", label = "닫기"              , command = self.menu_File_Quit)
        
        self.menuBar.add("cascade", label="파일", menu=top_File)
        ...
        top_View.add("command", label = "복귀   <F5>", command = self.menu_View_Refresh)
        top_View.add_separator()
        top_View.add_checkbutton(label = "레스터 이미지 보기"  ,  variable=self.include_Reng ,command= self.menu_View_Refresh)
        if DEBUG:
            top_View.add("command", label = "레스터 경로 계산", command = self.menu_Calc_Raster_Paths)
            top_View.add_checkbutton(label = "레스터 경로 표시" ,variable=self.include_Rpth ,command= self.menu_View_Refresh)
        top_View.add_checkbutton(label = "백터 각인 보기",   variable=self.include_Veng ,command= self.menu_View_Refresh)
        top_View.add_checkbutton(label = "백터 컷 보기"    ,   variable=self.include_Vcut ,command= self.menu_View_Refresh)
        top_View.add_checkbutton(label = "G-Code 경로 표시"  ,   variable=self.include_Gcde ,command= self.menu_View_Refresh)
        top_View.add_checkbutton(label = "가공 시간 예측",   variable=self.include_Time ,command= self.menu_View_Refresh)
        top_View.add_checkbutton(label = "디자인 이미지 확대",   variable=self.zoom2image   ,command= self.menu_View_Refresh)
        ...
        self.menuBar.add("cascade", label="보기", menu=top_View)


        top_USB = Menu(self.menuBar, tearoff=0)
        top_USB.add("command", label = "USB 재시작", command = self.Reset)
        top_USB.add("command", label = "USB 해제", command = self.Release_USB)
        top_USB.add("command", label = "레이져 초기화", command = self.Initialize_Laser)
        self.menuBar.add("cascade", label="USB", menu=top_USB)
        

        top_Settings = Menu(self.menuBar, tearoff=0)
        top_Settings.add("command", label = "일반 설정", command = self.GEN_Settings_Window)
        top_Settings.add("command", label = "레스터 이미지 설정",  command = self.RASTER_Settings_Window)
        top_Settings.add("command", label = "이미지 회전 설정",  command = self.ROTARY_Settings_Window)
        top_Settings.add_separator()
        top_Settings.add_checkbutton(label = "전문가 모드 설정", variable=self.advanced ,command= self.menu_View_Refresh)
        
        self.menuBar.add("cascade", label="설정", menu=top_Settings)
        
        top_Help = Menu(self.menuBar, tearoff=0)
        top_Help.add("command", label = "제작자(e-mail)", command = self.menu_Help_About)
        top_Help.add("command", label = "K40 Whisperer 웹페이지", command = self.menu_Help_Web)
        top_Help.add("command", label = "설명서 (웹페이지)", command = self.menu_Help_Manual)
        self.menuBar.add("cascade", label="도움말", menu=top_Help)
        ...
        try:
            ...
            debug_message('명령줄 옵션을 해석할 수 없음')
            sys.exit()
...
##                            plot_im = plot_im.filter(filter)
    ...
    def GEN_Settings_Window(self):
        ...
        #Radio Button
        D_Yloc=D_Yloc+D_dY
        self.Label_Units = Label(gen_settings,text="단위")
        ...
        D_Yloc=D_Yloc+D_dY
        self.Label_init_home = Label(gen_settings,text="초기화시 원점으로")
        ...
        D_Yloc=D_Yloc+D_dY
        self.Label_Preprocess_CRC = Label(gen_settings,text="CRC 데이터를 전처리")
        ...
        D_Yloc=D_Yloc+D_dY
        self.Label_Timeout = Label(gen_settings,text="USB 시간초과")
        self.Label_Timeout.place(x=xd_label_L, y=D_Yloc, width=w_label, height=21)
        self.Label_Timeout_u = Label(gen_settings,text="밀리초", anchor=W)
        ...
        D_Yloc=D_Yloc+D_dY
        self.Label_N_Timeouts = Label(gen_settings,text="시간 초과")
        ...
        self.Label_Inkscape_Path = Label(gen_settings,text="잉크스케이프 위치")
        ...
        self.Inkscape_Path = Button(gen_settings,text="잉크스케이프 찾기")
        ...
        D_Yloc=D_Yloc+D_dY
        self.Label_no_com = Label(gen_settings,text="오른쪽 상단이 원점")
        ...
        D_Yloc=D_Yloc+D_dY 
        self.Label_Board_Name      = Label(gen_settings,text="보드 이름", anchor=CENTER )
        ...
        D_Yloc=D_Yloc+D_dY
        self.Label_Laser_Area_Width = Label(gen_settings,text="레이저 가로 길이")
        ...
        D_Yloc=D_Yloc+D_dY
        self.Label_Laser_Area_Height = Label(gen_settings,text="레이저 세로 길이")
        ...
        D_Yloc=D_Yloc+D_dY
        self.Label_Laser_X_Scale = Label(gen_settings,text="x축 축척(배율)")
        ...
        D_Yloc=D_Yloc+D_dY
        self.Label_Laser_Y_Scale = Label(gen_settings,text="Y축 축척(배율)")
        ...
        self.Label_SaveConfig = Label(gen_settings,text="구성 파일")
        self.Label_SaveConfig.place(x=xd_label_L, y=D_Yloc, width=113, height=21)

        self.GEN_SaveConfig = Button(gen_settings,text="저장")
        ...
        self.GEN_Close = Button(gen_settings,text="닫기")
    ...
    def RASTER_Settings_Window(self):
        ...
        raster_settings.title('레스터 이미지 설정')
        ...
        D_Yloc=D_Yloc+D_dY
        self.Label_Rstep   = Label(raster_settings,text="스캔라인 단계", anchor=CENTER )
        ...
        D_Yloc=D_Yloc+D_dY
        self.Label_EngraveUP = Label(raster_settings,text="바닥쪽으로 각인")
        ...
        D_Yloc=D_Yloc+D_dY
        self.Label_Halftone = Label(raster_settings,text="하프톤 (Dither)")
        ...
        ############
        D_Yloc=D_Yloc+D_dY 
        self.Label_Halftone_DPI      = Label(raster_settings,text="하프톤 해상도", anchor=CENTER )
        ...
        self.Label_bezier_M1  = Label(raster_settings,
                                text="어두운쪽 조절 (%.1f)"%(self.bezier_M1_default),
                                anchor=CENTER )
        ...
        self.Label_bezier_M2  = Label(raster_settings,
                                text="밝은쪽 조절 (%.2f)"%(self.bezier_M2_default),
                                anchor=CENTER )
        ...
        self.Label_bezier_weight   = Label(raster_settings,
                                     text="전이 (%.1f)"%(self.bezier_M1_default),
                                     anchor=CENTER )
...
##            self.unsharp_t.trace_variable("w", self.Entry_Unsharp_Threshold_Callback)        
    ...
    def ROTARY_Settings_Window(self):
        ...
        rotary_settings.title('이미지 회전 설정')
        ...
        self.Label_Rotary_Enable = Label(rotary_settings,text="회전 설정 사용")
        ...
        D_Yloc=D_Yloc+D_dY
        self.Label_Laser_R_Scale = Label(rotary_settings,text="회전 축척 설정 (Y축)")
        ...
        D_Yloc=D_Yloc+D_dY
        self.Label_Laser_Rapid_Feed = Label(rotary_settings,text="빠른 속도 (기본값=0)")
    ...
    def EGV_Send_Window(self,EGV_filename):
        ...
        D_Yloc=D_Yloc+D_dY
        self.Label_Preprocess_CRC = Label(egv_send,text="CRC 데이터 사전 처리")
        self.Label_Preprocess_CRC.place(x=xd_label_L, y=D_Yloc, width=w_label, height=21)
        ...
        D_Yloc=D_Yloc+D_dY
        font_entry_width=215
        self.Label_Inkscape_Path = Label(egv_send,text="EGV 파일:")
        self.Label_Inkscape_Path.place(x=xd_label_L, y=D_Yloc, width=w_label, height=21)
        ...
        self.EGV_Close = Button(egv_send,text="취소")
        ...
        self.EGV_Send = Button(egv_send,text="EGV 데이터 전송",command=Close_and_Send_Click)
...
class UnitsDialog(tkSimpleDialog.Dialog):
    def body(self, master):
        ...
        Label(master, text="DXF 파일을 가져올 단위:").grid(row=0)
        Radio_Units_IN = Radiobutton(master,text="인치",        value="Inches")
        Radio_Units_MM = Radiobutton(master,text="밀리미터",   value="Millimeters")
        Radio_Units_CM = Radiobutton(master,text="센티미터",   value="Centimeters")
...
class pxpiDialog(tkSimpleDialog.Dialog):
    ...
    def body(self, master):
        ...
        ###########################################################################
        t0="열려는 SVG 파일이 있는 경우 이 대화 상자가 열림\n"
        t1="확인할 수 있는 정보가 충분하지 않음\n"
        t2="디자인의 의도 된 물리적 크기\n"
        t3="SVG 가져올 svg파일 배율을 선택하십시오:\n"
        ...
        Label_SVG_pxpi_96   = Label(master,text="(Inkscape v0.92 이상으로 저장된 파일)", anchor=W)
        
        Radio_SVG_pxpi_90   = Radiobutton(master,text=" 90 px/in", value="90")
        Label_SVG_pxpi_90   = Label(master,text="(Inkscape v0.91 이전 버전으로 저장된 파일)", anchor=W)
        
        Radio_SVG_pxpi_72   = Radiobutton(master,text=" 72 px/in", value="72")
        Label_SVG_pxpi_72   = Label(master,text="(Adobe Illustrator로 저장된 파일)", anchor=W)

        Radio_Res_Custom = Radiobutton(master,text=" 커스텀:", value="custom")
        ...
        Label_Width =  Label(master,text="가로", anchor=W)
        ...
        Label_Height =  Label(master,text="세로", anchor=W)
